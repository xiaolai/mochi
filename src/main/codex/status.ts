...
export type CodexStatus =
  ...
  | { readonly kind: 'logged-out'; readonly version: string }
  /** Codex thinks it is logged in; its access token is stale and only Codex can refresh it. */
  | { readonly kind: 'stale'; readonly version: string; readonly expiredAt: Date }
  | { readonly kind: 'unreadable'; readonly version: string; readonly reason: string }
  | {
      readonly kind: 'ready'
...
async function describeInstall(path: string, probe: CodexProbe): Promise<CodexStatus> {
  ...
  if (signedIn !== true) return signedIn === false ? { kind: 'logged-out', version } : signedIn

  return judgeToken(version, probe)
}
 ...
 */
async function judgeToken(version: string, probe: CodexProbe): Promise<CodexStatus> {
  ...
  switch (token.kind) {
    case 'absent':
      return { kind: 'logged-out', version }
    case 'unreadable':
      return { kind: 'unreadable', version, reason: token.reason }
    case 'expired':
      return { kind: 'stale', version, expiredAt: token.expiredAt }
    case 'api-key':
      return { kind: 'ready', version, mode: 'apikey' }
    case 'valid':
      return { kind: 'ready', version, mode, expiresAt: token.expiresAt }
  }
}
