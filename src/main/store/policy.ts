 ...
 *
 * Loudly, unlike the drill's progress: this decides whether a conversation is
 * written to disk, so a save that silently did nothing would leave the pane
 * showing a setting the app is not honouring. The rule is that when the
 * effective answer cannot be established, nothing is stored — and the caller
 * can only honour that if it learns the write failed.
 */
