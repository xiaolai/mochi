...
//
// RELATIVE, with the extension, not the `@shared` alias. `scripts/make-icons.ts`
// imports this module under bare `node`, which resolves neither -- and adding
// the alias here silently broke `pnpm icons` while every other gate stayed
// green, because nothing in `pnpm verify` runs the generator. There is a smoke
// test now, but the cheapest fix is to keep this file resolvable by anything.
import { SHEAR_GAIN } from '../../../shared/avatar-layout.ts'
